...
const Rotors = (props: Props) => {
  ...
  const fetchData = async () => {
    try { const res = await axios.get('/api/submersible'); setData(res.data) }
  ...
  useEffect(() => {
    ...
    let filtered = data
    if (props.calenderValue === 'daily') filtered = data.filter(d => new Date(d.date as string).toLocaleDateString() === now.toLocaleDateString())
    else if (props.calenderValue === 'weekly' || props.calenderValue === 'monthly') filtered = data.filter(d => new Date(d.date as string).getTime() >= startDate.getTime())
    else if (props.customDates?.[0] && props.customDates?.[1]) filtered = data.filter(d => new Date(d.date as string).getTime() >= props.customDates![0] && new Date(d.date as string).getTime() <= props.customDates![1])

    setFilteredData(filtered.sort((a, b) => new Date(b.date as string).getTime() - new Date(a.date as string).getTime()))
  }, [props.calenderValue, data, props.customDates])

  const confirmDelete = async () => {
    try {
      await axios.post('/api/deleteSub', { subId: deleteItem!.subId, clientName: deleteItem!.client })
      ...
      <Box sx={{ border: '1px solid #e2e8f0', borderRadius: 3, overflow: 'hidden', bgcolor: '#fff' }}>
        ...
        <Box sx={{ px: 2.5, py: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #f0f4f8', flexWrap: 'wrap', gap: 1 }}>
          <Box display="flex" alignItems="center" gap={1}>
            ...
            <Typography fontWeight={700}>Submersible Rotors</Typography>
            <Chip label={rows.length} size="small" sx={{ bgcolor: '#e3f2fd', color: '#0277bd', fontWeight: 700 }} />
          </Box>
          <TextField
            ...
            <TableBody>
              {rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, i) => (
                <TableRow key={row._id as string} hover sx={{ '&:last-child td': { border: 0 }, bgcolor: i % 2 === 0 ? '#fff' : '#fafafa' }}>
                  ...
                  <TableCell><Chip label={row.rotorSize as string} size="small" sx={{ bgcolor: '#e3f2fd', color: '#0277bd', fontWeight: 600, fontSize: 11 }} /></TableCell>
        ...
        <DialogContent>
          {deleteItem && (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
              ...
              <Typography variant="body2"><strong>Size:</strong> {deleteItem.rotorSize as string}</Typography>
